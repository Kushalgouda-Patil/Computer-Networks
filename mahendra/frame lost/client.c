...
#define LOSS_PROBABILITY 0.3 // Adjust this probability for frame loss
...
void receive_file(SOCKET client_socket) {
    ...
    while ((n = recv(client_socket, data, BUFFER_SIZE, 0)) > 0) {
        if ((double)rand() / RAND_MAX > LOSS_PROBABILITY) { // Simulate frame loss
            fwrite(data, 1, n, fp);
        } else {
            printf("Frame lost\n");
            continue; // Skip writing to the file if frame is lost
        }
    }
